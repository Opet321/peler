...
async def _owner(client: Client, message: Message): 
    ...
    if message.reply_to_message:
        ...
        if message_id:
            ...
            reply_message = await message.reply_text(f"<b><blockquote>terkirim ke {message_id['user_id']}</b></blockquote>", reply_to_message_ids=message.id, disable_notification=True)
...
async def _user(client: Client, message: Message): 
    ...
    user_db = await users.find_one({"user_id": f"{message.from_user.id}"})
    if not user_db:
        await message.reply_text("<b>You are not in the database, enter /start to use the bot!</b>", reply_to_message_ids=message.id)
    else:
