...
async def simpan_note(client, message):
    ...
    if not msg:
        return await message.edit_text("`Silakan balas ke pesan.`")
    ...
    await message.edit_text(f"**Berhasil menyimpan catatan dengan nama** `{name}`")
...
async def panggil_notes(client, message):
    ...
    if not _note:
        return await eor(message, text="Tidak ada catatan seperti itu.")
...
async def remove_notes(client, message):
    ...
    if deleted:
        await message.edit_text(f"**Berhasil Menghapus Catatan:** `{name}`")
    else:
        await message.edit_text(f"**Tidak dapat menemukan catatan:** `{name}`")
...
async def get_notes(client, message):
    ...
    if not _notes:
        return await message.edit_text("**Tidak ada catatan.**")
